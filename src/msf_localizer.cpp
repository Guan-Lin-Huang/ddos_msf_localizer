...
void Extended_Kalman_filter::init(){    
    ...
    this->_private_nh.param("GPS_novatel_heading",
        this->_gps_novatel_heading_topic_,std::string("/novatel/inspva"));  
    this->_private_nh.param("RTCM_Status_topic",
        this->_rtcm_status_topic_,std::string("/ddos_defender/rtcm_status_code"));  
    ...
    this->_gps_weight = 1.0;
    this->_rtcm_status_code = 0;
    ...
    this->_gps_novatel_heading_sub.shutdown();
...
void Extended_Kalman_filter::run(){
    ...
    if (this->_gps_mode_ == true){
        ...
        this->_gps_novatel_heading_sub = this->_nh.subscribe(this->_gps_novatel_heading_topic_.c_str(),1000,
            &Extended_Kalman_filter::_gps_novatel_heading_callback,this);  
    }
    ...
    if (this->_ndt_mode_ == true){
        ...
        this->_rtcm_status_topic_sub = this->_nh.subscribe(this->_rtcm_status_topic_.c_str(),1000,
            &Extended_Kalman_filter::_rtcm_stat_callback,this);    
    ROS_INFO_STREAM(CYAN<<" EKF : ready ! ");
};

void Extended_Kalman_filter::_rtcm_stat_callback(const std_msgs::Int32::ConstPtr& input) {
    this->_rtcm_status_code = input->data;
}
void Extended_Kalman_filter::_imu_callback(const sensor_msgs::Imu::ConstPtr input){
    ...
    ROS_INFO_STREAM(BOLDYELLOW <<" EKF : Covariance: "<< this->_gps_data.position_covariance[0]);
    ROS_INFO_STREAM(BOLDYELLOW <<" EKF : GPS weight: "<< this->_gps_weight);
...
void Extended_Kalman_filter::_gps_callback(const sensor_msgs::NavSatFix::ConstPtr input){
    ...
    double covariance = this->_gps_data.position_covariance[0];
    int status_code = this->_rtcm_status_code;
    double base_weight = 2* (1 - 1 / (1 + exp(-1 * (covariance - 0.008)))) ;

    // obtain final weight through status_code 
    if(status_code == 4){
        base_weight = base_weight - 1;
    }else{
        base_weight = base_weight - 0.1 * status_code;
    }   

    this->_gps_weight = std::max(0.0, std::min(base_weight, 1.0));
...
void Extended_Kalman_filter::_gps_rpy_callback(const ublox_msgs::NavATT::ConstPtr input){
    ...
    this->_gps_rpy.yaw = this->_gps_novatel_data.azimuth ;  
...
void Extended_Kalman_filter::_gps_info_callback(const ublox_msgs::NavPVT::ConstPtr input){
    ...
    if ( this->_gps_sat == false ){
        ...
        gps_sat_count = ( (gps_sat_count != 5) && (this->_gps_info_data.numSV > this->_ndt_gps_sat_threshold_))? gps_sat_count + 1 : 0;
    }else{
        this->_gps_sat = (this->_gps_info_data.numSV < this->_ndt_gps_sat_threshold_)? false : true;
    }
    this->_gps_info_initial_or_not = true;
    ROS_INFO_STREAM(BOLDMAGENTA <<" EKF : GPS info updated ! ");
}
void Extended_Kalman_filter::_gps_novatel_heading_callback(const novatel_gps_msgs::Inspva::ConstPtr input){
    this->_gps_novatel_data = *input;
    // this->_gps_rpy.yaw = this->_gps_novatel_data.azimuth ;
}
...
void Extended_Kalman_filter::_ndt_rel_callback(const std_msgs::Float32::ConstPtr input){
    ...
    if (total_count == 100){
        this->_ndt_weight = double((total_count-unstabel_count)*0.01);
...
void Extended_Kalman_filter::_Q_matrix_updated(){
    ...
    this->_Q_system_noise_cov(9,9) = 1e-10;
    this->_Q_system_noise_cov(10,10) = 1e-5;
    this->_Q_system_noise_cov(11,11) = 1e-9;
...
void Extended_Kalman_filter::_R_matrix_updated(bool gps_sat_flag){
    ...
    if (gps_sat_flag == false || this->_gps_weight < 0.5){
        ...
        ROS_INFO_STREAM(" EKF : R matrix updated ! :: NDT");
    }else{
        ...
        ROS_INFO_STREAM(" EKF : R matrix updated ! :: GPS");
    }

    this->_R_measurement_cov(6,6) = 1e0;
    this->_R_measurement_cov(7,7) = 1e0;
    this->_R_measurement_cov(8,8) = 1e-2;
...
void Extended_Kalman_filter::_GPS_matrix_updated(){
    ...
    if ( this->_ndt_mode_ * this->_gps_mode_ != 0 ){
        weight = this->_gps_weight;
        ...
        current_diff_3d = current_diff_2d +
        ...
        if ((!ndt_time_out) * this->_ndt_time_cost * this->_ndt_iteration * 
            this->_ndt_score * this->_ndt_msf_switch != 0) {
            this->_gps_measurement(0) =  weight * this->_gps_measurement(0) + (1 - weight)  * this->_ndt_data.pose.position.y;
            this->_gps_measurement(1) = weight * this->_gps_measurement(1) + (1 - weight)  * this->_ndt_data.pose.position.x;
            this->_gps_measurement(2) = weight * this->_gps_measurement(2) + (1 - weight)  * (-this->_ndt_data.pose.position.z);
            ROS_INFO_STREAM(BOLDYELLOW <<" EKF : In Fusion Case");
        }else{
            this->_gps_measurement(0) =  weight * this->_gps_measurement(0) + (1 - weight)  * this->_ndt_data.pose.position.y;
            this->_gps_measurement(1) = weight * this->_gps_measurement(1) + (1 - weight)  * this->_ndt_data.pose.position.x;
            this->_gps_measurement(2) = weight * this->_gps_measurement(2) + (1 - weight)  * (-this->_ndt_data.pose.position.z);
            ROS_INFO_STREAM(BOLDYELLOW <<" EKF : No In Fusion Case");
        }

        _R_matrix_updated(this->_gps_sat);
        ROS_INFO_STREAM(BOLDYELLOW <<" EKF : Fusion GPS weight: "<< this->_gps_weight);
        ROS_INFO_STREAM(BOLDMAGENTA <<" EKF : GPS sat "<<this->_gps_sat<< "! ");
    }

    // speed < 0.001  to stand still and lock position
    if ( this->_can_mode_){
        if ( this->_can_data.speed < 0.001 ){
...
void Extended_Kalman_filter::_Publisher(){
    ...
    if ( this->_tf_map_2_base_link_ == true){
        current_br_msf.sendTransform(tf::StampedTransform(msf_tf_n2b,now_time,"/map","/base_link"));
    }
    if ( this->_tf_map_2_msf_link_ == true){
        msf_br.sendTransform(tf::StampedTransform(msf_tf_n2b,now_time,"/map","/msf_link"));
    }

    // publish pose
    this->_msf_data.header.frame_id = "map";
    this->_msf_data.header.stamp = now_time;
    ...
    this->_msf_state_data.header.stamp = now_time;
    ...
    this->_msf_state_data.reliability = this->_gps_weight;
    ...
    this->_msf_fix_data.header.stamp = now_time;
